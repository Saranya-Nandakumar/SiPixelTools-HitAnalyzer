...
# 1) open your file
events = Events('file:myClustoldnew100.root')
...
for ievt, ev in enumerate(events):
    ...
    for detSet in clusters:
        detId = detSet.id()
        sorted_clusters = sorted(detSet, key=lambda cl: cl.size())
#        if(detId != 303075356 ):
#            continue

        totalPix=0
        # 5) loop clusters in that module
        for cid,cluster in enumerate(sorted_clusters):
            totalPix+=cluster.size()
            #print(cluster)
...
#                print(f"      row={pix.x:3d}  col={pix.y:3d}, adc={pix.adc}")
        print(f" Module detId = {detId}  ({detSet.size()} clusters) ({totalPix} hits)")
    if ievt >= 4:
        break
